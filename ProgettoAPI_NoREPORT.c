...
void relationCleanup(entitynode n, char* key) {
	if (n != NILL)
	{
		...
		rbDeleteRelation(&n->root, key, "");
		relationCleanup(n->right, key);
	}
}

void inorderTreeWalkRelation2(entitynode z, char* key, relationnode root)
{

	if (root != NILLR)
	{

		inorderTreeWalkRelation2(z, key, root->left);
		contmax--;
		entitynode a = treeSearch(ROOT, root->key);
		if (a != NILL && z != a)
		{

			rbDeleteRelation(&a->root, key, "");
		}

		inorderTreeWalkRelation2(z, key, root->right);
	}
}
