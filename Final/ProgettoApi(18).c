...
relationSummary getSummary1(relationSummary* head, char* key) {
...
int main() {
	...
	//char c[200];
	//FILE* fp;
	//fp = fopen("C:\\Users\\DAVO_\\Desktop\\batch5.2.in", "r");
	while (/*fgets(c, sizeof c, fp) != NULL*/ true)
	{
		/*c[strlen(c) - 1] = 0;*/
		char* c = malloc(1);
		int character;
		...
		{		 
			struct Report_Summary reportlist[ARRAYLENGHT];
			for (int j = 0;j < ARRAYLENGHT;j++)
			{
